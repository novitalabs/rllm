#!/usr/bin/env python3
"""
SWE-bench Docker Training Test
Minimal test using Docker containers for code execution.
Connects to existing inference server on port 8000.
"""
...
# Add rft-tinker to path for SWEBenchDockerEnvironment
sys.path.insert(0, "/root/work/rft-tinker")
from src.environments.swebench_docker_env import SWEBenchDockerEnvironment
...
def generate_response(messages: List[Dict]) -> str:
    """Call existing transformers server for inference."""
    try:
...
def execute_action(env: SWEBenchDockerEnvironment, action_type: str, args: str) -> str:
    ...
    if action_type == "bash":
        ...
        output, _ = env.run(f"grep -rn '{pattern}' {path} | head -100")
        return output[:5000]

    elif action_type == "find_file":
        output, _ = env.run(f"find {repo_path} -name '{args}' | head -50")
        return output[:2000]
...
def run_rollout(instance: Dict, max_steps: int = 50) -> Dict:
    """Run a single rollout for an instance using Docker backend."""
    instance_id = instance.get("instance_id", "unknown")
    logger.info(f"Starting rollout for {instance_id}")

    env = SWEBenchDockerEnvironment(ds=instance)
    ...
    try:
        ...
        while not done and step < max_steps:
            ...
            if action_type is None:
                ...
                logger.info(f"Action: {action_type} -> Observation: {obs[:200]}...")
