...
# =============================================================================
# DeepSWE Training Script for 32x H200 GPUs (4 nodes x 8 GPUs)
# Model: Qwen3-32B
# Target: Reproduce DeepSWE with 4x compute scale
#
# Prerequisites:
#   - Ray cluster running across 4 nodes (see setup_32h200_cluster.sh)
...
echo "=============================================="
echo "DeepSWE Training - 32x H200 (4 nodes x 8 GPUs)"
...
if ! python3 -c "import ray; ray.init(address='auto'); print(f'Ray cluster: {ray.cluster_resources()}'); ray.shutdown()" 2>/dev/null; then
    ...
    echo "  bash ppio/scripts/setup_32h200_cluster.sh ray-start"
    exit 1
fi
echo "Ray cluster OK."
...
# -----------------------------------------------------------------------------
# 32x H200 GPU Configuration (4 nodes x 8 GPUs)
# Total VRAM: 32 x 141GB = ~4.5TB
# Qwen3-32B BF16: ~64GB, fits with TP=8 within single node
# FSDP shards optimizer/gradients across all 32 GPUs
# -----------------------------------------------------------------------------
...
# Batch sizes - scaled 4x from single node (8 -> 32)
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen3-32b-32h200-v1}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-/data/checkpoints/deepswe-32h200/${EXPERIMENT_NAME}}"
...
python3 -m rllm.trainer.verl.train_agent_ppo \
    ...
    trainer.project_name='deepswe-32h200' \
