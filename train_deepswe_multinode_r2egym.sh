...
# DeepSWE Multi-node Training with R2EGym Agent
# Uses: enforce_eager=True, log_prob_micro_batch_size_per_gpu=1, r2egym agent
...
echo "=== DeepSWE Multi-node with R2EGym ==="
echo "Model: $MODEL"
echo "Backend: docker"
echo "GPUs: 16x H200 (2 nodes x 8)"
echo "Dataset: 32 cached samples"
echo "Scaffold: r2egym (9-step detailed workflow)"

python3 -m rllm.trainer.verl.train_agent_ppo \
    algorithm.adv_estimator=rloo \
    data.train_files=${RLLM_DIR}/rllm/data/datasets/R2E_Gym_Subset/train_cached.parquet \
    ...
    trainer.project_name='deepswe-r2egym' \
    trainer.experiment_name='multinode-2x8-r2egym' \
    ...
    2>&1 | tee ~/work/logs/deepswe_multinode_r2egym.log
