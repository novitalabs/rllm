...
# Force local Ray instance for TP=8
...
# CRITICAL: Force local Ray instance by unsetting any address
unset RAY_ADDRESS
export RAY_ADDRESS=""
...
echo "=== DeepSWE TP=8 Training (Local Ray) ==="
echo "Model: $MODEL"
echo "Backend: docker"
echo "Config: TP=8 (single-node), batch=4, n=8"
echo "Ray: Local instance (no multi-node)"

# Add ray_kwargs to force local initialization
python3 -m rllm.trainer.verl.train_agent_ppo \
    ...
    trainer.project_name=deepswe-tp8 \
    trainer.experiment_name=tp8-local \
    ...
    2>&1 | tee ~/work/logs/deepswe_tp8_forcelocal.log
